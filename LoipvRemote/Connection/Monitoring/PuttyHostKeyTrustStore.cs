...
{
    /// <summary>
    /// Validates an SSH host key against the key already accepted by PuTTYNG.
    /// This keeps the monitor non-interactive without silently trusting new keys.
    /// </summary>
    internal static class PuttyHostKeyTrustStore
    {
        ...
        internal static string? BuildEd25519CacheValue(string hostKeyName, byte[] hostKey)
        {
            if (!string.Equals(hostKeyName, "ssh-ed25519", StringComparison.Ordinal) ||
        ...
        internal static string? BuildRsaCacheValue(byte[] hostKey)
        {
