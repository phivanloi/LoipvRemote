...
==================================
- Create a new Database called "LoipvRemote" on your SQL Server.
- Give the users that you want to grant access to the LoipvRemote Connections Database Read/Write permissions on the Database.

Steps to configure LoipvRemote for SQL
====================================
- Start LoipvRemote if it's not already running.
...
- Click OK to apply the changes. The main window title should now change to "LoipvRemote | SQL Server".
...
- You should now be able to do everything you were able to do with the XML storage plus see the changes live on another LoipvRemote instance that is connected to the same Database.
