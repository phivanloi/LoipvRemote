...
where you can personalize your options for all of LoipvRemote.
This includes how to set logging, credentials and so on.
Continue reading for the details of the different options here.

Startup/Exit
============
Options below are for the various settings for Startup/Exit of LoipvRemote.
...
.. list-table::
   ...
   * - Allow only a single instance of the application (LoipvRemote restart required)
     - Off
     - Enforces and makes sure only a single instance of LoipvRemote is running on the computer

Appearance
==========
Various options for LoipvRemote appearance.
...
.. list-table::
   ...
   * - Language
     ...
     - Which language to use for the interface of LoipvRemote
   ...
   * - Show full connections file path in window title
     - Off
     - Adds the complete path to the title of LoipvRemote to where the connection file is located
   * - Always show notification area icon
     - Off
     - Adds LoipvRemote to the taskbar in the OS
   * - Minimize to notification area
     - Off
     - Will place LoipvRemote in taskbar on minimize

Tabs & Panels
=============
Various settings for how tabs & panels should work in LoipvRemote.
...
.. list-table::
   ...
   * - Always show panel tabs
     - Off
     - Will always show the tabs & panels in LoipvRemote
   * - Open new tab to the right of the currently selected tab
     - On
     - When active then open next tab on the right of the active selection in LoipvRemote. Turn this off and next tab will open the next connection at the end of all tabs
   ...
   * - Create a empty panel when LoipvRemote starts
     - Off
     - On startup if this is active LoipvRemote will create a panel mentioned under Panel Name
...
.. list-table::
   ...
   * - Set hostname like display name when creating or renaming connections
     - Off
     - Will make LoipvRemote try to use the remote host hostname to set the title of the tab in LoipvRemote.
   ...
   * - When closing connections Warn me...
     ...
     - Various options of how LoipvRemote should act when you close connections. The different options are listed below:
       ::

           - ... when any connection closes
           - ... when closing multiple connections
           - ... only when exiting LoipvRemote
           - ... never
   ...
   * - Connection Backup Frequency
     - On Edit
     - Various options of when LoipvRemote should create a backup of the connections file. The different options are listed below:
       ::
...
Credentials
===========
Options for credentials in LoipvRemote. The main purpose here is that when you have empty username, password or domain field then use below information.
...
Updates
=======
Options for how LoipvRemote should check for updates from the website.
...
.. list-table::
   ...
   * - Check for updates at startup
     - On (Every 14 days)
     - Here you can choose how often LoipvRemote checks for updates. Standard is every 14 days
   * - Release Channel
     - Stable
     - The main channel to use for LoipvRemote. Note that the channels are described under the selection. Stable is suggested for normal usage but its always good to get feedback on upcoming releases
...
Theme
=====
This is not enabled by default but can be used inside LoipvRemote.
To enable themes you have to first enable it in the checkbox at the bottom of the options.
Then restart LoipvRemote in order for it to work.
