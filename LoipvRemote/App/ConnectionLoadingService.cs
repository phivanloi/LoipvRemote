    ...
    ConnectionImportService connectionImportService)
{
    ...
    public void LoadConnections(bool withDialog = false)
    {
        ...
        try
        {
            _connectionsService.RemoteConnectionsSyncronizer?.Disable();
            ...
            {
                connectionFileName = _connectionsService.GetStartupConnectionFileName();
            }

            _connectionsService.LoadConnections(OptionsDBsPage.Default.UseSQLServer, false, connectionFileName);
            if (OptionsDBsPage.Default.UseSQLServer)
                _connectionsService.LastSqlUpdate = DateTime.Now.ToUniversalTime();
            else
                _connectionsService.LastFileUpdate = File.GetLastWriteTime(connectionFileName);
        ...
        catch (Exception exception)
        {
            ...
            if (exception is FileNotFoundException && !withDialog)
            {
                ...
                while (!answered)
                {
                    try
                    {
                        ...
                        {
                            case 0:
                                _connectionsService.NewConnectionsFile(connectionFileName);
                                answered = true;
                                break;
                            case 1:
                                LoadConnections(true);
                            ...
                            case 2:
                                _connectionsService.NewConnectionsFile(connectionFileName);
                                _connectionImportService.ImportFromFile(_connectionsService.ConnectionTreeModel.RootNodes[0]);
                                answered = true;
                                break;
                            case 3:
                                Application.Exit();
            ...
            if (connectionFileName != _connectionsService.GetStartupConnectionFileName())
            {
                LoadConnections(withDialog);
            }
            else
            {
                _connectionWorkspace.ShowError(
                    string.Format(Language.ErrorStartupConnectionFileLoad, Environment.NewLine, Application.ProductName,
                        _connectionsService.GetStartupConnectionFileName(), MiscTools.GetExceptionMessageRecursive(exception)),
