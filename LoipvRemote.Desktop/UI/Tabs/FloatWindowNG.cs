...
namespace LoipvRemote.UI.Tabs
{
    class FloatWindowNG : FloatWindow
    {
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            const int wmSysCommand = WindowsShellWindowMessages.WmSysCommand;
            ...
            if (m.Msg == wmNcLButtonDown)
            {
                ...
                if ((uint)m.WParam == 8) // Check if button down occured in minimize box
                {
                    if (WindowState == FormWindowState.Minimized)
                        _ = WindowsShellWindowMessages.SendMessage(Handle, wmSysCommand, (IntPtr)SC_RESTORE, IntPtr.Zero);
                    else
                        _ = WindowsShellWindowMessages.SendMessage(Handle, wmSysCommand, (IntPtr)SC_MINIMIZE, IntPtr.Zero);
