...
namespace LoipvRemote.App
{
    ...
    public static class ProgramRoot
    {
        ...
        private static async Task MainAsync(string[] args)
        {
            ...
            using IHost host = DesktopApplicationHost.Create(args, ApplicationServiceRegistration.Register);
            // Keep the process identity stable so Windows resolves the taskbar
            // button against LoipvRemote's icon.
            host.Services.GetRequiredService<IApplicationIdentityService>().Configure(AppUserModelId);
            try
            {
        ...
        private static void StartApplicationAsSingleInstance(IServiceProvider services)
        {
            ...
            {
                SwitchToCurrentInstance(services);
                return;
            }
        ...
        private static void SwitchToCurrentInstance(IServiceProvider services)
        {
            ...
            services.GetRequiredService<IWindowActivationService>().RestoreAndActivate(singletonInstanceWindowHandle);
        }
        ...
        private static void ShowSplashOnStaThread()
        {
            ...
            {
                AppStartupState.Splash = FrmSplashScreen.GetInstance();

                AppStartupState.Splash.StartPosition = FormStartPosition.CenterScreen;
                Application.Run(AppStartupState.Splash);
            })
            { IsBackground = true };
