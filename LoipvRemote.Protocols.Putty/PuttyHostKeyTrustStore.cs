...
/// <summary>
/// Validates a monitor-channel host key against the key that the user already
/// accepted in PuTTY. The monitor never auto-trusts a new server key.
/// </summary>
internal static class PuttyHostKeyTrustStore
{
    ...
    internal static void PreferCachedHostKeyAlgorithms(ConnectionInfo connectionInfo, string hostname, int port)
    {
    ...
    private static string? BuildEd25519CacheValue(string hostKeyName, byte[] hostKey)
    {
        if (!string.Equals(hostKeyName, "ssh-ed25519", StringComparison.Ordinal) ||
    ...
    private static string? BuildRsaCacheValue(byte[] hostKey)
    {
    ...
    private static bool TryReadSshString(byte[] data, int offset, out byte[] value, out int nextOffset)
    {
        ...
        if (length < 0 || data.Length - offset - 4 < length)
            return false;
