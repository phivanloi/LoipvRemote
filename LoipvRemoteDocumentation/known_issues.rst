...
LoipvRemote bundles a legacy PuTTY version (below 1.77.2)
=======================================================

LoipvRemote bundles puttyNG, a customized fork to improve the interoperability between Putty and LoipvRemote. As this bundling process happens with the building process, puttyNG can not be updated in the installer.
...
PuttyNG can be updated manually with the following steps:
    ...
    1. Retrieve the newest PuttyNG version from `github.com/LoipvRemote/PuTTYNG/releases <https://github.com/LoipvRemote/PuTTYNG/releases>`_
    2. Replace the local PuTTYNG.exe in the LoipvRemote installation directory

Problems connecting to servers using SSH with LoipvRemote version below 1.77.2
============================================================================

LoipvRemote bundles puttyNG, a customized fork to improve the interoperability between Putty and LoipvRemote. Older Putty versions might cause connection issues, especially for Ubuntu 22 or other servers that expect PubkeyAcceptedAlgorithms. To fix this issue, either a LoipvRemote update to 1.77.2 or newer is required or the PuTTYNG.exe has to be replaced manually as follows:
...
LoipvRemote uses the Microsoft Terminal Services Client (MSTSC) libraries
in order to make Remote Desktop connections.

.. note::

    LoipvRemote has no control over the functionality changes implemented by Microsoft.
...
Log4net vulnerability CVE-2018-1285 (LoipvRemote version below 1.77.2)
====================================================================
...
2. Copy log4net.dll from net40 folder into LoipvRemote install folder (default *C:\\Program Files (x86)\\LoipvRemote\\* )

3. Edit *LoipvRemote.exe.config* and add the following section under the assembly binding for ``WeifenLuo.WinFormsUI.Docking``
...
ATI Tray Tools
==============
LoipvRemote is not compatible with ATI Tray Tools. We are aware of the issue and
hope to have it fixed in a future version. We recommend that you disable or
uninstall ATI Tray Tools while using LoipvRemote.

LoipvRemote crashes with the error "Class not registered" when trying to connect using RDP
========================================================================================
You may also see a message like "System.Runtime.InteropServices.COMException (0x80040154)"

If you are running LoipvRemote on Windows 7 or Server 2008:
...
If you are running LoipvRemote on Windows 8/10 or Server 2012+:

- Try to repair the LoipvRemote installation using the installer or uninstall/reinstall. Receiving this error on these OS's is just an install fluke (or you've fiddled with your registry).
...
========================================================================================================================================================================
RFB version 4.0 and higher is a proprietary version owned by `RealVNC Limited <https://www.realvnc.com/>`_. Building support for newer versions will likely result in licensing fees. Therefore, it is unlikely that LoipvRemote will have support for version 4.0+ anytime soon.
...
when LoipvRemote passes the value to the cmd line, which then invokes PuTTY.
This was investigated in issue `#186 <https://github.com/LoipvRemote/LoipvRemote/issues/186>`_
...
===================================================
Your RDP connection reconnects after resizing LoipvRemote or the connection panel.
...
This is a known issue with The Microsoft RDP library that cannot be solved by LoipvRemote.
