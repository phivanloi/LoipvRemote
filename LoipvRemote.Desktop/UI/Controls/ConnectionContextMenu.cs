...
namespace LoipvRemote.UI.Controls
{
    ...
    public sealed class ConnectionContextMenu : ContextMenuStrip
    {
        ...
        private ToolStripMenuItem _cMenTreeShowPassword = null!;
        ...
        internal void ShowHideMenuItemsForConnectionNode(ConnectionInfo connectionInfo)
        {
            ...
            if (!(connectionInfo.Protocol == ProtocolKind.Rdp))
            {
                _cMenTreeConnectWithOptionsConnectInFullscreen.Enabled = false;
                _cMenTreeConnectWithOptionsConnectToConsoleSession.Enabled = false;
                _cMenTreeClearCachedRdpCredentials.Enabled = false;
            ...
            if (connectionInfo.Protocol == ProtocolKind.ExternalApplication)
                _cMenTreeConnectWithOptionsNoCredentials.Enabled = false;

            if (connectionInfo.Protocol != ProtocolKind.Rdp && connectionInfo.Protocol != ProtocolKind.Vnc)
                _cMenTreeConnectWithOptionsViewOnly.Enabled = false;
        ...
        private void OnClearCachedRdpCredentialsClicked(object? sender, EventArgs e)
        {
            ...
            if (selected == null || selected.Protocol != ProtocolKind.Rdp) return;
        ...
        private void OnImportFileClicked(object? sender, EventArgs e)
        {
            ...
            if (_connectionTree.SelectedNode == null)
                selectedNodeAsContainer = ShellRuntime.ConnectionTreeWorkspace.ConnectionTreeModel.RootNodes.First();
            else
        ...
        private void OnExportFileClicked(object? sender, EventArgs e)
        {
            ShellRuntime.ConnectionExportService.ExportToFile(
                _connectionTree.SelectedNode,
                ShellRuntime.ConnectionTreeWorkspace.ConnectionTreeModel);
        }
        ...
        {
            if (sender is ToolStripMenuItem { Tag: ExternalTool externalTool })
